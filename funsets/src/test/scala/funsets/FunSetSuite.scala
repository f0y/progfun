...
class FunSetSuite extends FunSuite {
  ...
  test("intersect") {
    new TestSets {
      ...
      assert(!contains(intersection, 3), "interesection 3")
    }
  }
  ...
  test("filter") {
    new TestSets {
      val s123 = union(union(union(s1, s2), s3), singletonSet(1000))
      val filtered = filter(s123, x => x <= 2)
      printSet(filtered)
  ...
  test("for all") {
    new TestSets {
      ...
      assert(!forall(s123, x => x > 1), "forall 1")
      assert(forall(s123, x => x <= 3), "forall 2")
    }
  }
  ...
  test("exist") {
    new TestSets {
      val s123 = union(union(s1, s2), s3)
      assert(!exists(s123, x => x > 3), "exist 1")
      assert(exists(s123, x => x == 3), "exist 2")
    }
  }
  ...
  test("map squared") {
    new TestSets {
      ...
      assert(contains(s149, 1), "map 1")
      assert(contains(s149, 4), "map 2")
      assert(contains(s149, 9), "map 3")
      assert(!contains(s149, 2), "map 4")
    }
  }
  ...
  test("map edge case") {
    new TestSets {
      ...
      val s1000 = map(s, x => x)
      assert(contains(s1000, 1000), "map 1")
    }
  }
