...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.filter(x => x.isLetter).groupBy(x => x).toList.sortWith(_._1 < _._1).map({case (k, v) => (k, v.length)})
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    case List() => List(List())
    case head :: tail => combinations(tail) ++ (for {
      combinations <- combinations(tail)
      count <- 1 until head._2 + 1
    } yield (head._1, count) :: combinations)
  }
   ...
   */
  def subtract(first: Occurrences, second: Occurrences): Occurrences =
    first.map(x => second.find(x._1 == _._1) match {
